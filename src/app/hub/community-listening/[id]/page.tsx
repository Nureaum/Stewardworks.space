...
import { ChevronLeft, Loader2, Calendar, MapPin, Volume2, BookOpen, Image as ImageIcon, Video, FileText, Music } from 'lucide-react';
import Link from 'next/link';

export default function CommunitySessionDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    console.log("Fetching session:", params.id);
    fetch(`/api/public/community-sessions/${params.id}?t=${Date.now()}`)
      .then(res => res.json())
      .then(data => {
        console.log("DEBUG - FETCHED SESSION DATA:", data.session);
        console.log("DEBUG - MEDIA ARRAY:", data.session?.media);
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-[#FDFDFD] flex flex-col items-center justify-center p-8 font-exo text-center">
        ...
        <h1 className="text-3xl font-black text-steward-dark uppercase tracking-tighter mb-4">Session Not Found</h1>
        <p className="text-steward-dark/60 mb-8 max-w-md">The community session you are looking for does not exist or has been removed.</p>
        <Link 
          href="/hub/community-listening"
  ...
  const images = session.media?.filter((m: any) => m.media_type === 'image') || [];
  const videos = session.media?.filter((m: any) => m.media_type === 'video_link') || [];
  const pdfs = session.media?.filter((m: any) => m.media_type === 'pdf') || [];
  const audios = session.media?.filter((m: any) => m.media_type === 'external_link') || [];
  
  // Parse embedded location and date from the title string (e.g., "Title|||Location|||Date")
  ...
  // Use the first image as the header background if there's no thumbnail
  const headerBgUrl = session.thumbnail_url || (images.length > 0 ? images[0].url : null);
  ...
  try {
    formattedDate = new Date(displayDateStr).toLocaleDateString(undefined, {
      ...
      month: 'long',
      day: 'numeric',
      timeZone: 'UTC' // Prevent off-by-one day issues if date is YYYY-MM-DD
    });
  } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] font-exo pb-24">
      {/* Header exactly like Env Literacy */}
      <header className="relative w-full bg-steward-dark overflow-hidden pt-12 pb-16">
        {headerBgUrl ? (
          <>
            <img src={headerBgUrl} alt={session.title} className="absolute inset-0 w-full h-full object-cover opacity-30" />
        ...
        <div className="w-full mx-auto px-8 md:px-16 relative z-10">
          <Link href="/hub/community-listening" className="inline-flex items-center gap-2 text-steward-gold font-bold text-xs uppercase tracking-widest hover:text-white transition-colors mb-12">
      ...
      <main className="w-full mx-auto px-8 md:px-16 py-12 relative z-20 space-y-12 max-w-5xl">
        ...
        {/* Summary Tab */}
        {activeTab === 'summary' && session.body && (
            ...
            {audios.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {audios.map((audio: any, idx: number) => {
                  const isAudioFile = audio.url.match(/\.(mp3|wav|ogg|m4a|webm)$/i) || audio.url.includes('/storage/v1/object/public/');
                  return (
                    <div key={audio.id || idx} className="flex flex-col gap-4 bg-steward-offwhite rounded-2xl p-4 border border-steward-dark/5 shadow-sm">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 shrink-0 rounded-full bg-steward-blue/10 flex items-center justify-center text-steward-blue">
                          <Volume2 size={20} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-steward-dark text-sm truncate">{audio.label || `Recording ${idx + 1}`}</p>
                          <p className="text-[10px] text-steward-dark/50 font-black uppercase tracking-widest mt-0.5">
                            {isAudioFile ? 'Audio File' : 'External Link'}
                          </p>
                        </div>
                      </div>
                      {isAudioFile ? (
                        ...
                        <a href={audio.url} target="_blank" rel="noopener noreferrer" className="w-full py-2 bg-white rounded-lg border border-gray-200 text-xs font-bold text-steward-dark text-center hover:bg-gray-50 transition-colors">
                          Open Link ↗
                        </a>
                      )}
                    </div>
