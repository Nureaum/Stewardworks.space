...
export interface SortableListProps<T extends { id: string }> {
  ...
  renderItem: (item: T, isDragging: boolean, dragHandleProps: React.HTMLAttributes<HTMLElement>) => React.ReactNode;
}

export function SortableList<T extends { id: string }>({ items, onChange, renderItem }: SortableListProps<T>) {
  const [activeId, setActiveId] = useState<string | null>(null);

  // Only PointerSensor — no KeyboardSensor so Space/Enter in inputs never triggers drag
