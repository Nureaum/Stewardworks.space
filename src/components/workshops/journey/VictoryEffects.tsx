...
// Extracted from original HTML prototype
...
export function getCelebrateProps() {
  ...
  // gentle rising sparks in the sky region
...
export function getWinSkill(id: string) {
  ...
  switch (id) {
    ...
    case 'quest':
      tone = P;
      fx = [{ style: { position: 'absolute', left: '50%', top: '86px', width: '120px', height: '120px', marginLeft: '-60px', marginTop: '-60px', border: `3px dashed ${P}`, borderRadius: '50%', opacity: 0.75, animation: 'skSpin 6s linear infinite', boxShadow: `0 0 16px ${P}` } }, beam(P, 50, 54, 60, 2, 0.5)];
...
export function buildCastFx(id: string, n: number) {
  ...
  const ray = (k: string, ang: number, c: string, dur: number, delay?: number) => (
    <div key={k} style={{ ...B, left: cx, top: cy, width: '6px', height: '70px', marginLeft: '-3px', marginTop: '-35px', transformOrigin: 'center center', '--ang': `${ang}deg`, borderRadius: '3px', background: `linear-gradient(180deg,${c},transparent)`, boxShadow: `0 0 10px ${c}`, animation: `cRay ${dur}s ease-out ${delay || 0}s both` }} />
  );
